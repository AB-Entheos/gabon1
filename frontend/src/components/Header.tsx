...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-20 flex h-[72px] shrink-0 items-center justify-between border-b border-slate-200 bg-white/80 px-6 backdrop-blur-header dark:border-slate-800 dark:bg-slate-900/80">
      <div>
        <div className="text-[11px] font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">
          {t("nav.dashboard", "Dashboard")}
        </div>
        <h1 className="text-xl font-bold text-slate-900 dark:text-slate-100">{t("app.name", "HEC Emergency Fund")}</h1>
      </div>
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-2.5 py-1 text-xs dark:border-slate-700 dark:bg-slate-800">
          {online ? (
            <>
